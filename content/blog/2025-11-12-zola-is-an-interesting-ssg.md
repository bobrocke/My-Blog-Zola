...
date: 2025-11-12
slug: 2025-11-12-zola-is-an-interesting-ssg
taxonomies:
...
Hugo, written in Go, may be the fastest of the SSGs in building a site. But Zola, written in Rust, seems every bit as fast. Neither disappoint and build so fast you almost don't notice. This site builds in 100ms or less with Zola.

Blogs are not very complicated websites, so Zola has what you need to create one. I think Zola should be added to [Hugo and Eleventy](@/blog/2025-08-27-hugo-or-11ty.md) when looking for an SSG for your blog. It's a little simpler than Hugo, so is missing a few things that Hugo does well.
...
- Support -- Zola is basically a one-man show, which means Vincent does the best he can responding to questions and problems. Support for Hugo comes from their Discourse Forum and is fast, detailed, and super helpful.
...
So there's going to be a bit more text wrangling required to migrate blog posts if you use taxonomies.
...
Because Zola is developed by a very small team, it doesn't see frequent updates. That's good from a breakage point of view, but also means that if Zola doesn't do something you need done, you can't really depend on an update to provide it.

But if you're looking for a batteries-included static site generator for your blog, Zola has to be ranked up there with Hugo.
